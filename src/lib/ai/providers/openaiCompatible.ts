...
/**
 * Any OpenAI-compatible chat endpoint. Covers Groq, OpenRouter, Cerebras,
 * Mistral, Together, a local Ollama, and so on. One adapter, many free tiers.
 */
...
export interface CompatibleConfig {
  ...
  /** Explicit model from env; wins when set. */
  model: () => string | undefined;
  /** Ordered guesses when no model is set and listing fails. */
  defaultModels: string[];
  /** Whether to send `response_format: json_object`. */
...
/** Rank chat models by likely capability; null means "not a chat model". */
...
export function createCompatibleProvider(cfg: CompatibleConfig): Provider {
  ...
  async function resolveModel(key: string): Promise<string> {
    ...
    console.info(`Ideako: using ${cfg.label} model "${model}".`);
    return model;
  }
  ...
  return {
    ...
    async complete(opts) {
      ...
      // Retired or mistyped model: try the next known default once.
      ...
      if (!res.ok) {
        ...
        if (res.status === 400 || res.status === 401 || res.status === 403) {
          return { ...err("upstream", `${cfg.label} rejected the request (${res.status})${reason ? `: ${reason}` : ""}.`), model };
        }
