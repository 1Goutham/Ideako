 ...
 * Model resolution.
 *
 * Gemini model IDs are retired regularly (gemini-2.0-flash was shut down on
 * 2026-06-01), so Ideako never hardcodes one. It asks the API which models
 * this key can use, prefers the newest stable Flash model, and caches the
 * answer. `GEMINI_MODEL` still wins when it's set and available.
 */

const BASE = (process.env.GEMINI_API_BASE || "https://generativelanguage.googleapis.com").replace(/\/$/, "");
const CACHE_MS = 60 * 60 * 1000;
/** Used only when ListModels itself cannot be reached. */
...
export function scoreModel(name: string): number | null {
  ...
  if (/-\d{2,}$/.test(suffix)) score -= 1; // dated snapshot; prefer the rolling stable ID
  return score;
}
...
export function pickModel(available: string[], preferred?: string): string | null {
  ...
  const alias = available.find((n) => /^gemini-flash-latest$/.test(n)) ?? available.find((n) => /^gemini-.*-latest$/.test(n));
  return alias ?? null;
}
...
export async function resolveModel(key: string): Promise<string> {
  ...
  if (available === null) {
    ...
    console.warn(`Ideako: could not list Gemini models; falling back to "${model}".`);
  } else {
    const picked = pickModel(available, preferred);
    if (preferred && picked !== preferred) {
      console.warn(`Ideako: GEMINI_MODEL="${preferred}" is not available for this key; using "${picked ?? LAST_RESORT}" instead.`);
    }
    model = picked ?? preferred ?? LAST_RESORT;
  }

  cache = { model, at: Date.now() };
  console.info(`Ideako: using Gemini model "${model}".`);
  return model;
}
