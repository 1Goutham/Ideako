...
/**
 * Google Gemini. Best free tier for this workload: no card required, strong
 * instruction following, native JSON mode. Model IDs are resolved from the
 * API because Google retires them regularly.
 */
...
export const gemini: Provider = {
  ...
  async complete(opts) {
    ...
    if (!res.ok) {
      ...
      if (res.status === 400 || res.status === 401 || res.status === 403) {
        return { ...err("upstream", `Gemini rejected the request (${res.status})${reason ? `: ${reason}` : ""}.`), model };
      }
    ...
    if (payload.promptFeedback?.blockReason) return { ...err("blocked", "Gemini declined this request. Try rephrasing your idea."), model };

    const candidate = payload.candidates?.[0];
    const text = candidate?.content?.parts?.map((p) => p.text ?? "").join("").trim();
    if (!text) {
      if (candidate?.finishReason === "SAFETY") return { ...err("blocked", "Gemini declined this request. Try rephrasing your idea."), model };
  ...
  async status(): Promise<ProviderStatus> {
    ...
    return {
      ...
      note: "Free tier, no card. Best quality for this workload.",
    };
  },
};
